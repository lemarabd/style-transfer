...
style_dir = "data"
content_dir = "Assignment/DeepLabV3/data/leftImg8bit/train"

# # load all images in the style directory and content directory (subfolders)
# style_imgs = []
# content_imgs = []

# for filename in os.listdir(style_dir):
#     style_imgs.append(image_loader(os.path.join(style_dir, filename)))

# for filename in os.listdir(content_dir):
#     content_imgs.append(image_loader(os.path.join(content_dir, filename)))
...
# # resize all style images to the same size as the content images
# for i in range(len(style_imgs)):
#     if style_imgs[i].size() < content_imgs[i].size():
#         content_imgs[i] = transforms.Resize(style_imgs[i].size()[2:])(content_imgs[i])
#     elif style_imgs[i].size() > content_imgs[i].size():
#         style_imgs[i] = transforms.Resize(content_imgs[i].size()[2:])(style_imgs[i])

#     assert style_imgs[i].size() == content_imgs[i].size(), \
#         "we need to import style and content images of the same size"
